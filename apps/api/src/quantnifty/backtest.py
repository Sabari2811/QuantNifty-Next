...
def _find_leg(snapshot: dict[str, Any], instrument: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    for row in rows:
        ...
        if sid and str(row.get("security_id") or "") == sid:
            exact.append(row)
        elif symbol and str(row.get("trading_symbol") or "") == symbol:
            exact.append(row)
        elif strike and abs(_f(row.get("strike")) - strike) < 0.001 and (not side or str(row.get("side") or "").upper() == side):
            exact.append(row)
...
def run_backtest(snapshots: list[dict[str, Any]], strategy: str = "directional", config: BacktestConfig | None = None) -> dict[str, Any]:
    ...
    while i < len(ordered) - 1:
        decision = final_decision(ordered[i], previous, mode)
        previous = ordered[i]
