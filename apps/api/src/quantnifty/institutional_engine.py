...
def volatility_engine(data: dict[str, Any], previous: dict[str, Any] | None = None) -> dict[str, Any]:
    prev=previous or {}; iv=_f(data.get("atm_iv")); piv=_f(prev.get("atm_iv")); skew=data.get("iv_skew"); pskew=prev.get("iv_skew"); em=_f((data.get("expected_move") or {}).get("move")); pem=_f((prev.get("expected_move") or {}).get("move")); iv_change=_pct(iv,piv) if piv else 0; em_change=_pct(em,pem) if pem else 0
...
def risk_engine(data: dict[str, Any], signal: dict[str, Any], strategy: str="directional", mode: str="LIVE") -> dict[str, Any]:
    state=((data.get("intelligence") or {}).get("market_state") or {}).get("state") or ""; replay=mode.upper() in {"BACKTEST","REPLAY"}; gates={"direction":signal.get("direction") in {"BULLISH","BEARISH"},"confidence":_f(signal.get("confidence"))>=60,"liquidity":_f(data.get("liquidity_score"))>=50,"market_state":state not in {"LIQUIDITY_RISK","COMPRESSION"},"data_integrity":data.get("data_integrity")=="LIVE_PROVIDER" or (replay and data.get("data_integrity")=="RECORDED_HISTORICAL")}
    ...
    reasons=[k for k,ok in gates.items() if not ok]; return {"strategy":strategy,"mode":mode.upper(),"gates":gates,"approved":not reasons,"reasons":reasons,"max_risk_pct":.5 if strategy=="gamma_blast" else 1.0}

def execution_plan(data: dict[str, Any], signal: dict[str, Any], risk: dict[str, Any]) -> dict[str, Any]:
    approved=bool(risk.get("approved")); direction=signal.get("direction","NEUTRAL"); selections=data.get("strike_selection") or []; selections=selections.get("candidates") or selections.get("strikes") or [] if isinstance(selections,dict) else selections; chosen=selections[0] if selections else None; spot=_f(data.get("spot")); em=_f((data.get("expected_move") or {}).get("move")); stop=max(em*.35,spot*.002) if em else spot*.002
...
def final_decision(data: dict[str, Any], previous: dict[str, Any] | None=None, strategy: str="directional", mode: str="LIVE") -> dict[str, Any]:
    signal=institutional_signal(data,previous); risk=risk_engine(data,signal,strategy,mode); plan=execution_plan(data,signal,risk); return {"signal":signal,"risk":risk,"execution_plan":plan,"status":"TRADE_CANDIDATE" if risk["approved"] else "NO_TRADE","authoritative":"FINAL_DECISION","trading":"DISABLED","mode":mode.upper()}
...
def replay_signal_stack(snapshots: list[dict[str, Any]], mode: str="REPLAY") -> dict[str, Any]:
    ...
    candidates=sum(r["decision"]["status"]=="TRADE_CANDIDATE" for r in results); return {"count":len(results),"trade_candidates":candidates,"no_trade":len(results)-candidates,"results":results,"signal_neutral":False,"orders_placed":0}
