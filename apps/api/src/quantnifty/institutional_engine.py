...
from quantnifty.research_brain import strategy_selector
...
def gamma_flip_detector(data: dict[str, Any], previous: dict[str, Any] | None = None) -> dict[str, Any]:
    spot = _f(data.get("spot")); flip = data.get("gamma_flip"); prev = previous or {}
    prev_spot = _f(prev.get("spot")); prev_flip = prev.get("gamma_flip")
...
def oi_flow_analyzer(data: dict[str, Any]) -> dict[str, Any]:
    ...
    for r in data.get("option_chain") or []:
        flow = _flow_label(r); counts[flow] = counts.get(flow, 0) + 1; side = str(r.get("side") or "").upper()
        doi = _f(r.get("oi")) - _f(r.get("previous_oi")); weight = abs(doi) * max(_f(r.get("last_price")), 1.0)
        notional[flow] = notional.get(flow, 0) + weight
...
def volatility_engine(data: dict[str, Any], previous: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    em = _f((data.get("expected_move") or {}).get("move")); pem = _f((prev.get("expected_move") or {}).get("move")); iv_change = _pct(iv, piv) if piv else 0; em_change = _pct(em, pem) if pem else 0
...
def institutional_signal(data: dict[str, Any], previous: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    raw = {"BULLISH": 0.0, "BEARISH": 0.0, "NEUTRAL": 0.0}; evidence = []; base = str(data.get("bias") or "NEUTRAL").upper()
    ...
    if ev["probability_confidence"] >= 70 and abs(ev["bullish_probability"] - ev["bearish_probability"]) >= 35:
        direction = "BULLISH" if ev["bullish_probability"] > ev["bearish_probability"] else "BEARISH"; confidence = max(confidence, ev["probability_confidence"])
...
def _adaptive_signal(data: dict[str, Any], previous: dict[str, Any] | None, signal: dict[str, Any]) -> dict[str, Any]:
    selection = strategy_selector(data, previous)
    preferred = str(selection.get("preferred_direction") or "NEUTRAL").upper()
    ...
    if preferred in {"BULLISH", "BEARISH"}:
        if direction == "NEUTRAL":
            direction = preferred
            confidence = max(confidence, _f(selection.get("confidence")))
        elif direction != preferred:
            direction = "NEUTRAL"
    adaptive = dict(signal)
    adaptive["direction"] = direction
    adaptive["confidence"] = round(confidence, 1)
    adaptive["adaptive"] = {"regime": selection["regime"], "selected_strategy": selection["selected_strategy"], "preferred_direction": preferred, "readiness_pct": selection["confidence"], "reason": selection["reason"]}
    return adaptive


def risk_engine(data: dict[str, Any], signal: dict[str, Any], strategy: str = "directional", mode: str = "LIVE") -> dict[str, Any]:
    strategy = str(strategy or "directional").lower(); state = ((data.get("intelligence") or {}).get("market_state") or {}).get("state") or ""; replay = mode.upper() in {"BACKTEST", "REPLAY"}
    ...
    if strategy == "gamma_blast":
        gates["gamma_regime"] = signal.get("gamma", {}).get("regime") == "NEGATIVE"; gates["volatility"] = signal.get("volatility", {}).get("regime") == "VOL_EXPANSION"
    elif strategy == "adaptive":
        selected = ((signal.get("adaptive") or {}).get("selected_strategy") or "standby").lower()
        if selected == "gamma_blast":
            gates["gamma_regime"] = signal.get("gamma", {}).get("regime") == "NEGATIVE"; gates["volatility"] = signal.get("volatility", {}).get("regime") == "VOL_EXPANSION"
        elif selected == "transition":
            gates["gamma_transition"] = ((signal.get("adaptive") or {}).get("regime") == "GAMMA_TRANSITION")
        elif selected in {"range", "breakout_watch", "standby"}:
            gates["strategy_entry"] = False
    reasons = [k for k, ok in gates.items() if not ok]
    return {"strategy": strategy, "mode": mode.upper(), "selected_strategy": selected, "gates": gates, "approved": not reasons, "reasons": reasons, "max_risk_pct": .5 if strategy == "gamma_blast" or selected == "gamma_blast" else 1.0}
...
def execution_plan(data: dict[str, Any], signal: dict[str, Any], risk: dict[str, Any]) -> dict[str, Any]:
    ...
    if selections:
        ...
        chosen = next((item for item in selections if wanted and str(item.get("side") or item.get("option_type") or "").upper() == wanted), None) if wanted else selections[0]
        chosen = chosen or selections[0]
...
def final_decision(data: dict[str, Any], previous: dict[str, Any] | None = None, strategy: str = "directional", mode: str = "LIVE") -> dict[str, Any]:
    ...
    if requested not in {"directional", "gamma_blast", "adaptive"}:
