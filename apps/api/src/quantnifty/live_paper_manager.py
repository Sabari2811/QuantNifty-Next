...
class LivePaperManager:
    ...
    def __init__(self) -> None:
        self.sequence = 0; self.active: PaperTrade | None = None; self.entry_price = 0.0; self.entry_quantity = 1; self.instrument: dict[str, Any] | None = None; self.entry_reasons: dict[str, Any] = {}; self.entry_decision: dict[str, Any] = {}
        self._recover()
    ...
    def _restore(self, outcome: dict[str, Any]) -> None:
        ...
        self.entry_price = _f(outcome.get("entry_price")); self.entry_quantity = max(1, int(outcome.get("quantity", 1))); self.instrument = outcome.get("instrument") if isinstance(outcome.get("instrument"), dict) else None; self.entry_reasons = outcome.get("entry_reasons") if isinstance(outcome.get("entry_reasons"), dict) else {}; self.entry_decision = outcome.get("entry_decision") if isinstance(outcome.get("entry_decision"), dict) else {}
    ...
    def _recover(self) -> None:
        ...
        # Never carry an old-day paper position into the current day. Reconcile
        # it at the final stored LIVE_PROVIDER snapshot from its own session so
        # its realized P&L is based on the last same-day market evidence rather
        # than a fabricated next-day price.
    ...
    def _open(self, snapshot: dict[str, Any], decision: dict[str, Any]) -> None:
        ...
        if not timestamp or spot <= 0 or direction not in {"BULLISH", "BEARISH"} or price <= 0:
            return
        self.sequence += 1; strategy = str((signal.get("adaptive") or {}).get("selected_strategy") or decision.get("strategy") or "adaptive"); quantity = _quantity(plan, instrument)
        ...
        record_outcome({**asdict(self.active), "day": trading_day(timestamp), "lifecycle": "OPEN", "entry_price": round(price, 6), "entry_price_source": "ASK_THEN_LAST_THEN_BID", "quantity": quantity, "instrument": self.instrument, "entry_reasons": self.entry_reasons, "entry_decision": self.entry_decision, "read_only": True, "execution": "NONE"})
    ...
    def _close(self, snapshot: dict[str, Any], decision: dict[str, Any], reason: str) -> dict[str, Any] | None:
        ...
        outcome.update({"day": trading_day(self.active.entry_timestamp), "entry_price": round(self.entry_price, 6), "exit_price": round(exit_price, 6), "exit_price_source": "BID_THEN_LAST_THEN_ASK" if exit_price > 0 else "UNAVAILABLE_SPOT_PROXY", "quantity": self.entry_quantity, "instrument": self.instrument, "entry_reasons": self.entry_reasons, "entry_decision": self.entry_decision, "exit_reasons": _exit_reasons(decision, reason), "exit_decision": decision, "gross_pnl_proxy": round(gross, 4), "realized_pnl": round(gross, 4), "pnl_basis": "OPTION_PREMIUM_WHEN_AVAILABLE_ELSE_SPOT_PROXY", "lifecycle": "CLOSED", "read_only": True, "execution": "NONE"})
