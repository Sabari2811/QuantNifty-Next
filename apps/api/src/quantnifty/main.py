...
app = FastAPI(title="QuantNifty Next", version="1.6.0")
...
def analytics(spot: float, rows: list[dict[str, Any]], expiry: str | None = None) -> dict[str, Any]:
    ...
    call_doi=sum(r["oi"]-r["previous_oi"] for r in calls); put_doi=sum(r["oi"]-r["previous_oi"] for r in puts)
    ...
    volume=sum(r["volume"] for r in rows); spread_cost=sum(max(0.0,r["ask"]-r["bid"]) for r in rows if r["ask"]>0 and r["bid"]>0)
    ...
    strikes=sorted(by_strike); support=max((s for s in strikes if s<=spot),default=None); resistance=min((s for s in strikes if s>=spot),default=None)
    ...
    score=max(0.0,min(100.0,score)); bias="BULLISH" if score>=60 else "BEARISH" if score<=40 else "NEUTRAL"; confidence=50.0 if bias=="NEUTRAL" else min(99.0,50.0+abs(score-50.0))
...
async def continuous_market_refresh():
    """Refresh the canonical market snapshot continuously while the instance is running."""
    while True:
        try:
            await snapshot()
        except Exception:
...
async def decision(strategy: str="directional"):
    ...
    em=(data.get("expected_move") or {}).get("move"); bias=data["bias"]; directional_ok=bias in {"BULLISH","BEARISH"}; confidence_ok=data["confidence"]>=60.0; liquidity_ok=data["liquidity_score"]>=50.0; gf=data.get("gamma_flip")
    gamma_position_ok=gf is not None and ((bias=="BULLISH" and data["spot"]>gf) or (bias=="BEARISH" and data["spot"]<gf)); dealer_ok=(bias=="BULLISH" and data["dealer_flow"]=="PUT_SUPPORT") or (bias=="BEARISH" and data["dealer_flow"]=="CALL_RESISTANCE")
    previous=cache.get("previous_snapshot") or {}; pg,cg=previous.get("gex"),data.get("gex"); gamma_acceleration=pg is not None and cg is not None and abs(cg)>abs(pg)*1.10; old_em=(previous.get("expected_move") or {}).get("move") if isinstance(previous.get("expected_move"),dict) else None; move_expansion=em is not None and (old_em is None or em>old_em*1.05); volume_ok=data.get("rows",0)>=20 and sum(r.get("volume",0) for r in data.get("option_chain",[]))>0
    gamma_blast_qualified=all([directional_ok,confidence_ok,liquidity_ok,gamma_position_ok,dealer_ok,gamma_acceleration,move_expansion,volume_ok]); selection=select_strikes(data["spot"],data["option_chain"],bias,gamma_blast=(mode=="gamma_blast"),expected_move=em,gamma_blast_qualified=gamma_blast_qualified); trade_allowed=directional_ok and confidence_ok and liquidity_ok and selection["eligible"]
