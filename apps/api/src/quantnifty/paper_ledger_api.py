...
def _outcomes_for_day(day: str) -> list[dict[str, Any]]:
    ...
    for event in load_events("outcomes"):
        ...
        if candidate_day != day:
            continue
        row = dict(outcome); row["timestamp"] = event.get("timestamp") or row.get("exit_timestamp") or row.get("entry_timestamp"); rows.append(row)
    return rows
...
def _open_rows(day: str) -> list[dict[str, Any]]:
    ...
    return sorted(latest_by_trade.values(), key=lambda row: str(row.get("entry_timestamp") or row.get("timestamp") or ""))
...
def _dynamic_open_row(row: dict[str, Any], latest_snapshot: dict[str, Any] | None) -> dict[str, Any]:
    ...
    result["quantity"] = quantity; result["entry_value"] = round(entry * quantity, 4) if entry > 0 else 0.0; result["mark_price"] = None; result["mark_source"] = "UNAVAILABLE"; result["mark_timestamp"] = None; result["unrealized_pnl"] = None; result["unrealized_pnl_pct"] = None; result["current_spot"] = None
    if latest_snapshot:
...
def _decision_summary(day: str) -> dict[str, Any]:
    events = load_events("decisions", day); directions: Counter[str] = Counter(); names: Counter[str] = Counter(); approved = blocked = 0; latest: dict[str, Any] | None = None
    for event in events:
...
def paper_ledger(day: str | None = Query(default=None, description="IST trading day YYYY-MM-DD; defaults to today")) -> dict[str, Any]:
    ...
    realized = sum(_num(row.get("realized_pnl", row.get("gross_pnl_proxy"))) for row in closed); spot_proxy = sum(_num(row.get("spot_move_proxy")) for row in closed); unrealized_values = [row.get("unrealized_pnl") for row in open_rows if row.get("unrealized_pnl") is not None]; unrealized = sum(_num(value) for value in unrealized_values); total_pnl = realized + unrealized; option_rows = [row for row in closed if str(row.get("pnl_basis") or "").startswith("OPTION_PREMIUM") and _num(row.get("exit_price")) > 0]

    stale_open_count = 0
    for outcome in _latest_open_lifecycle().values():
