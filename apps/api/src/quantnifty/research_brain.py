...
def pre_move_state(snapshot: dict[str, Any], previous: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    vol = _volume(snapshot); prev_vol = _volume(previous)
    iv = _f(snapshot.get("atm_iv")); prev_iv = _f(previous.get("atm_iv"))
    gex = _f(snapshot.get("gex")); prev_gex = _f(previous.get("gex"))
    ...
    bull_oi, bear_oi = _oi_pressure(snapshot); pressure_total = bull_oi + bear_oi
    ...
    readiness = 30.0 if compression else 0.0; readiness += 35.0 if pressure else 0.0; readiness += 35.0 if trigger else 0.0; readiness += 10.0 if gamma_shift else 0.0
