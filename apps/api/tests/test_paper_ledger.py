...
def _decision(approved=True):
    return {
        ...
        "signal": {
            "direction": "BULLISH",
            "confidence": 82,
            "evidence": ["market structure BULLISH", "OI flow BULLISH"],
            "rationale": ["gamma support"],
            "adaptive": {
                "regime": "TREND",
                "selected_strategy": "directional",
        ...
        "risk": {
            "approved": approved,
            "gates": {"direction": True, "confidence": True},
            "reasons": [] if approved else ["confidence"],
        },
        "execution_plan": {
            "instrument": {"security_id": "123", "trading_symbol": "NIFTY-23500-CE", "strike": 23500, "side": "CE", "lot_size": 65},
            "quantity": 65,
        },
    }
...
def _snapshot(ts="2026-09-10T04:00:00+00:00", spot=23500, bid=110, ask=112):
    return {
        ...
        "spot": spot,
        "option_chain": [{
            "security_id": "123", "trading_symbol": "NIFTY-23500-CE", "strike": 23500, "side": "CE",
            "bid": bid, "ask": ask, "last_price": 111,
        }],
    }
...
def test_trading_day_and_session_close():
    ...
    assert not session_close_required("2026-09-10T10:00:00+00:00")  # 15:30 IST boundary is UTC 10:00
...
def test_ledger_marks_open_position_dynamically(monkeypatch):
    open_row = {
        "trade_id": "paper-test", "status": "OPEN", "strategy": "directional", "direction": "BULLISH",
        "entry_timestamp": "2026-09-10T04:00:00+00:00", "entry_spot": 23500,
        "entry_price": 100, "quantity": 65,
        "instrument": {"security_id": "123", "trading_symbol": "NIFTY-23500-CE", "strike": 23500, "side": "CE"},
    }
    monkeypatch.setattr(ledger_module, "load_events", lambda kind, day=None: [
        {"timestamp": open_row["entry_timestamp"], "outcome": open_row}
    ] if kind == "outcomes" else [])
...
def test_ledger_recovers_legacy_same_day_rows_without_day_field(monkeypatch):
    closed = {
        "trade_id": "paper-legacy", "status": "CLOSED", "direction": "BULLISH",
        "entry_timestamp": "2026-09-10T04:00:00+00:00", "exit_timestamp": "2026-09-10T04:10:00+00:00",
        "entry_price": 100, "exit_price": 108, "quantity": 65, "gross_pnl_proxy": 520,
    }
    monkeypatch.setattr(ledger_module, "load_events", lambda kind, day=None: [
        {"timestamp": closed["exit_timestamp"], "outcome": closed}
    ] if kind == "outcomes" else [])
