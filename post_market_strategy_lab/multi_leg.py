...
from .runner import LabConfig, IST, _f, _session, _spot, _timestamp, _fill, _cost, _metrics
...
def _rows(snapshot:dict[str,Any],side:str)->list[dict[str,Any]]:
    return [r for r in snapshot.get("option_chain") or [] if isinstance(r,dict) and str(r.get("side") or r.get("option_type") or "").upper()==side and _f(r.get("last_price") or r.get("ltp"))>0]
...
def _strike_step(rows:list[dict[str,Any]])->float:
    strikes=sorted({_f(r.get("strike")) for r in rows if _f(r.get("strike"))>0}); diffs=[b-a for a,b in zip(strikes,strikes[1:]) if b>a]; return min(diffs) if diffs else 50.0
...
def _costs(legs:list[tuple[str,dict[str,Any]]],cfg:LabConfig,action:str)->float:
    total=0.0
    for side,row in legs:
        trade_side=side if action=="ENTRY" else ("BUY" if side=="SELL" else "SELL"); price=_fill(row,trade_side); total += _cost(price,cfg.lot_size,cfg,trade_side)
    return total
...
def run_multi_leg_tournament(snapshots:list[dict[str,Any]],cfg:LabConfig|None=None)->dict[str,Any]:
    ...
    return {"schema_version":"post-market-lab-multi-v2","research_only":True,"orders_placed":0,"configuration":asdict(cfg),"tests":len(results),"leaderboard":[{"rank":i+1,"strategy":r["strategy"],**r["metrics"]} for i,r in enumerate(results)],"results":results}
