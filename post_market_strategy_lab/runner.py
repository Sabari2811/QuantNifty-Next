...
class LabConfig:
    ...
    max_hold_bars: int=8
    stop_pct: float=.0075
    target_pct: float=.015
    slippage_bps: float=5.0
    fixed_cost_per_side: float=20.0  # brokerage/order by default
    transaction_charge_rate: float=.0003553  # NSE equity options, on premium
    sebi_rate: float=.000001          # Rs10/crore
    stamp_rate: float=.00003          # buyer side
    stt_sell_rate: float=.0015        # from 1-Apr-2026
    gst_rate: float=.18
...
def _option_key(row:dict[str,Any])->tuple[str,str,float,str]:return (str(row.get("security_id") or row.get("instrument_id") or ""),str(row.get("trading_symbol") or row.get("symbol") or ""),_f(row.get("strike")),str(row.get("expiry") or row.get("expiry_date") or ""))
...
def _select_option(snapshot:dict[str,Any],direction:str,variant:str="ATM")->dict[str,Any]|None:
    ...
    return min(rows,key=lambda r:abs(abs(_f(r.get("delta")))-target)) if target is not None else rows[0]
...
def _fill(row:dict[str,Any],action:str)->float:
    ...
    return (ask or last or bid) if action=="BUY" else (bid or last or ask)


def _cost(price:float,qty:int,cfg:LabConfig,action:str)->float:
    premium=abs(price*qty); brokerage=cfg.fixed_cost_per_side; txn=premium*cfg.transaction_charge_rate; sebi=premium*cfg.sebi_rate; stamp=premium*cfg.stamp_rate if action=="BUY" else 0.0; stt=premium*cfg.stt_sell_rate if action=="SELL" else 0.0; gst=cfg.gst_rate*(brokerage+txn+sebi)
    slippage=premium*cfg.slippage_bps/10000.0
    return brokerage+txn+sebi+stamp+stt+gst+slippage


def _metrics(trades:list[dict[str,Any]],cfg:LabConfig)->dict[str,Any]:
    pnls=[float(t["net_pnl"]) for t in trades]; wins=[p for p in pnls if p>0]; losses=[p for p in pnls if p<0]; gp,gl=sum(wins),abs(sum(losses)); equity=cfg.initial_capital; peak=equity; dd=0.0; by_day:dict[str,float]={}
    for t in trades:
...
def run_single_leg_tournament(snapshots:list[dict[str,Any]],cfg:LabConfig|None=None)->dict[str,Any]:
    ...
    return {"schema_version":"post-market-lab-v3","mode":"RAW_LIVE_SESSION_COUNTERFACTUAL","research_only":True,"orders_placed":0,"configuration":asdict(cfg),"strategy_count":len(all_strategies()),"variant_count":len(variants),"tests":len(results),"leaderboard":[{"rank":i+1,"strategy":r["strategy"],"variant":r["variant"],**r["metrics"]} for i,r in enumerate(results)],"results":results}
