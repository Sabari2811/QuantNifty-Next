...
@dataclass
class LabConfig:
    initial_capital:float=100000.;lot_size:int=65;max_hold_bars:int=8;stop_pct:float=.0075;target_pct:float=.015;slippage_bps:float=5.;fixed_cost_per_side:float=20.;transaction_charge_rate:float=.0003553;sebi_rate:float=.000001;stamp_rate:float=.00003;stt_sell_rate:float=.0015;gst_rate:float=.18;min_volume:float=0.
def _f(v:Any)->float:
...
def _expiry_date(v:Any)->date|None:
    text=str(v or "").strip()
...
def _front_expiry(rows:list[dict[str,Any]])->str|None:
    vals=[_expiry_value(r.get("expiry") or r.get("expiry_date")) for r in rows if r.get("expiry") or r.get("expiry_date")]
    if not vals:return None
    parsed=[(_expiry_date(v),v) for v in vals];valid=[x for x in parsed if x[0] is not None]
    return min(valid,key=lambda x:x[0])[1] if valid else sorted(vals)[0]
...
def _select_option(snapshot:dict[str,Any],direction:str,variant:str="ATM")->dict[str,Any]|None:
    ...
    if variant=="OTM":
        x=[r for r in rows if (_f(r.get("strike"))>spot if side=="CE" else _f(r.get("strike"))<spot)];return min(x,key=lambda r:abs(_f(r.get("strike"))-spot)) if x else rows[0]
    target={"D050":.5,"D060":.6,"D070":.7}.get(variant);return min(rows,key=lambda r:abs(abs(_f(r.get("delta")))-target)) if target is not None else rows[0]
...
def _cost(price:float,qty:int,cfg:LabConfig,action:str)->float:
    premium=abs(price*qty);brokerage=cfg.fixed_cost_per_side;txn=premium*cfg.transaction_charge_rate;sebi=premium*cfg.sebi_rate;stamp=premium*cfg.stamp_rate if action=="BUY" else 0.;stt=premium*cfg.stt_sell_rate if action=="SELL" else 0.;gst=cfg.gst_rate*(brokerage+txn+sebi);slippage=premium*cfg.slippage_bps/10000.;return brokerage+txn+sebi+stamp+stt+gst+slippage
def _metrics(trades:list[dict[str,Any]],cfg:LabConfig)->dict[str,Any]:
    ...
    for t in trades:equity+=t["net_pnl"];peak=max(peak,equity);dd=max(dd,peak-equity);by_day[t["day"]]=by_day.get(t["day"],0)+t["net_pnl"]
...
def run_single_leg_tournament(snapshots:list[dict[str,Any]],cfg:LabConfig|None=None)->dict[str,Any]:
    ...
    return {"schema_version":"post-market-lab-v5","mode":"RAW_LIVE_SESSION_COUNTERFACTUAL","research_only":True,"orders_placed":0,"configuration":asdict(cfg),"strategy_count":len(all_strategies()),"variant_count":len(variants),"tests":len(results),"leaderboard":[{"rank":i+1,"strategy":r["strategy"],"variant":r["variant"],**r["metrics"]} for i,r in enumerate(results)],"results":results}
