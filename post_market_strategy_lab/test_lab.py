...
class PostMarketLabSmokeTest(unittest.TestCase):
    def setUp(self):
        self.snaps=[snapshot(f"2026-09-15T09:{20+i*10:02d}:00+05:30",25000+i*10,100-i*5,90-i*2) for i in range(5)]
    ...
    def test_single_leg_tournament_is_research_only(self):
        ...
        self.assertEqual(report["tests"],72)
        self.assertTrue(all(r["contract_policy"]=="FIXED_AT_ENTRY" for r in report["results"]))

    def test_multi_leg_tournament_is_separate(self):
        report=run_multi_leg_tournament(self.snaps,LabConfig(max_hold_bars=2))
        self.assertTrue(report["research_only"])
        self.assertEqual(report["orders_placed"],0)
        self.assertEqual(report["tests"],8)
